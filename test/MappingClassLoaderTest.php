...
class MappingClassLoaderTest extends TestCase
{
    ...
    public function testInitialRun(){
        $this->makeClean();
        $foo = new Foo();
        self::assertTrue($this->sourceChangeResolver->didResolveClass(Foo::class));
        self::assertEquals('~~~[replaced-value]~~~', $foo->getValue());
        self::assertFileExists($this->sourceChangeResolver::classNameToCacheFilename(Foo::class));
        $bar = new Bar();
    ...
    public function testSubsequentRun(){
        $foo = new Foo();
        self::assertFalse($this->sourceChangeResolver->didResolveClass(Foo::class));
    }
    ...
    public function testXdebug()
    {
        ...
        self::assertEquals(
            $this->sourceChangeResolver::classNameToSourceFilename(Foo::class),
            $file
        );
    }
    ...
    {
        if (!FileHelper::rmDir($this->sourceChangeResolver::getCacheDirectory(), false)) {
            throw new Exception('Cannot delete directory ' . $this->sourceChangeResolver::getCacheDirectory());
        }
    }
